...
describe('FeatureMark.js', () => {
  test('stores latlng object on click ', () => {
    ...
    expect(wrapper.vm.$store.commit).toHaveBeenCalledTimes(3);
    expect(wrapper.vm.$store.commit.mock.calls[0][0]).toEqual(UPDATE_SHOULD_TEXT_SCROLL);
    expect(wrapper.vm.$store.commit.mock.calls[1][0]).toEqual(UPDATE_HIGHLIGHTED_BBOX);
    expect(wrapper.vm.$store.commit.mock.calls[2][0]).toEqual(UPDATE_HIGHLIGHTED_LAT_LNG);
  });
